	...
	*/
	...
	public void execute() {
		...
		for (Item2d item : viewResult.getItems()) {
			if (IntStream.of(item.getWidth()).sum() < 0) {
				if ((resultMax == -1) ||
						(IntStream.of(viewResult.getItems().get(resultMax).getWidth()).sum() <
								IntStream.of(item.getWidth()).sum())) {
					resultMax = idx;
				}
			} else {
				if ((resultMin == -1) ||
						(IntStream.of(viewResult.getItems().get(resultMin).getWidth()).sum() >
						IntStream.of(item.getWidth()).sum())) {
					resultMin = idx;
				}
			}
		...
		if (resultMin > -1) {
			System.out.print("Min positive #" + resultMin + " found: "
					+ String.format("%.2f.", viewResult.getItems().get(resultMin).getWidth()));
		} else {
			System.out.print("Min positive not found.");
		}
		if (resultMax > -1) {
			System.out.println(" Max negative #" + resultMax + " found: "
					+ String.format("%.2f.", viewResult.getItems().get(resultMax).getWidth()));
		} else {
